package com.teamblackhole.realtimechat.embedlibgdx.basketball.water;
...
public class Water implements Disposable {
    ...
    private Set<Map.Entry<Fixture, Fixture>> fixturePairs; // contacts between this object and other dynamic bodies
    ...
    private float tension = 2f;
    private float dampening = 0.05f;
    private float spread = 0.5f;
    private float density = 0f;

    private final float columnSparation = 0.02f; // 4 px between every column
    ...
    public Water(boolean waves, boolean splashParticles) {
        ...
        this.fixturePairs = new HashSet<Map.Entry<Fixture, Fixture>>();
        this.setDebugMode(false);

        if (waves) {
            textureWater = new TextureRegion(new Texture(Gdx.files.internal("new/basket-net.png")));
            polyBatch = new PolygonSpriteBatch();
        }

        if (splashParticles) {
            textureDrop = new Texture(Gdx.files.internal("new/drop.png"));
    ...
    public void update() {
        if (body != null && fixturePairs != null) {
            ...
            for (Map.Entry<Fixture, Fixture> pair : fixturePairs) {
    ...
    public Set<Map.Entry<Fixture, Fixture>> getFixturePairs() {
        return fixturePairs;
    }

    public void setFixturePairs(Set<Map.Entry<Fixture, Fixture>> fixturePairs) {
